...
sim_season_diagnostic <- function(true.season.box, team.a.id, year, n.steps.per.game=310, n.iter=1000, regular.season.only=TRUE) {
  ### Returns data.frame with sims as rows and state counts' SQUARED DIFFERENCE from true value as columns 
  # true.season.box := retrieved using get_true_team_box() from diagnostics.R
  # team.a.id := integer value ESPN ID for team of interest
  # year := year of interest (needs to be same year as that used to get true.season.box -- YYYY format)
